...
{
	$file_name="SimProxySvr_log.tar.gz";
	$SimProxySvr_name="SimProxySvr.log";
	$file_path="/tmp/log/";
	exec("tar -cvf ".$file_path.$file_name." ".$file_path.$SimProxySvr_name);
...
function Log_setting($log_class, $sim_log_autoclean, $sim_log_maxsize){
	$aql = new aql();
	...
	if(!isset($res['sim_log'])){
		$aql->assign_addsection('sim_log','');
	}
	
	if($log_class != ''){
		if(isset($res['sim_log']['log_class'])){
			$aql->assign_editkey('sim_log', 'log_class', $log_class);
		}else{
			$aql->assign_append('sim_log', 'log_class', $log_class);
		}
		$aql->save_config_file('logfile_monitor.conf');
	}
	
	if($sim_log_autoclean != ''){
		if(isset($res['sim_log']['autoclean'])){
			$aql->assign_editkey('sim_log','autoclean',$sim_log_autoclean);
		}else{
			$aql->assign_append('sim_log','autoclean',$sim_log_autoclean);
		}
	}
	
	if($sim_log_maxsize != ''){
		if(isset($res['sim_log']['maxsize'])){
			$aql->assign_editkey('sim_log','maxsize',$sim_log_maxsize);
		}else{
			$aql->assign_append('sim_log','maxsize',$sim_log_maxsize);
		}
	}
	$aql->save_config_file('logfile_monitor.conf');
	unlock_file($hlock);
	...
	$result = $client->__soapCall('SBKSetLogClass', array($log_class), array('location' => 'http://127.0.0.1:8888', 'uri' => 'webproxy'));
}
...
function Show_log_level(){
	...
	$res = $aql->query("select * from logfile_monitor.conf");
	if(isset($res['sim_log']['log_class']) && $res['sim_log']['log_class'] != ''){
		$log_class = $res['sim_log']['log_class'];
	}
	return $log_class;
}
$log_class = Show_log_level();
...
if($_POST) {
	if(isset($_POST['send'])) {
		if($_POST['send'] == 'Download') {
			...
			if(isset($_POST['log_class']) && $_POST['log_class'] != ''){
				$log_class = $_POST['log_class'];
			}else{
				$log_class = '';
			}
			
			if(isset($_POST['sim_log_autoclean']) && $_POST['sim_log_autoclean'] == 'on'){
				$sim_log_autoclean = $_POST['sim_log_autoclean'];
				$sim_log_maxsize = $_POST['sim_log_maxsize'];
			}else{
				$sim_log_autoclean = 'off';
				$sim_log_maxsize = '';
			}
			Log_setting($log_class, $sim_log_autoclean, $sim_log_maxsize);
		}
	}
}
...
$res = $aql->query("select * from logfile_monitor.conf");
$sim_log_autoclean = $res['sim_log']['autoclean'];
$sim_log_maxsize = $res['sim_log']['maxsize'];
unlock_file($hlock);
?>
	...
	<table width="100%" class="tedit" >
		<tr>
			...
			<td >
				<select id="log_maxsize" name="log_class" >
					<option value='0' <?php if($log_class == 0){echo "selected";}?>><?php echo language('Info');?></option>
					<option value='1' <?php if($log_class == 1){echo "selected";}?>><?php echo language('Warning');?></option>
					<option value='2' <?php if($log_class == 2){echo "selected";}?>><?php echo language('Error');?></option>
				</select>
			</td>				
		</tr>
			...
			<td >
				<table><tr>
					<td style="margin:0px;padding:0px;border:0">
						<input type=checkbox id="sim_log_autoclean" name="sim_log_autoclean" <?php if($sim_log_autoclean == 'on')echo 'checked';?> >
					</td>
					<td style="border:0">
						<?php echo language('maxsize');?> : 
						<select id="sim_log_maxsize" name="sim_log_maxsize" <?php if($sim_log_autoclean != "on")echo "disabled";?>>
							<?php
								$value_array = array("20KB","50KB","100KB","200KB","500KB","1MB","2MB");
								foreach($value_array as $value){
									$selected = "";
									if($sim_log_maxsize == $value)
										$selected = "selected";
	...
	<div id="tab" style="margin-top:15px;">
		<li class="tb1">&nbsp;</li>
		<li class="tbg"><?php echo language('SimProxySvr Log');?></li>
		<li class="tb2">&nbsp;</li>
	</div>
	<center>
		...
		<table>
			<tr>	
				...
				<td>
					<button onclick="document.getElementById('send').value='Download';"><?php echo language('Download');?></button>
				</td>
			</tr>
		</table>
	</center>
...
$(document).ready(function(){
	...
	$(":checkbox").iButton(); 
	$("#sim_log_autoclean").change(function(){$("#sim_log_maxsize").attr("disabled", !$("#sim_log_autoclean").attr("checked"));});
}); 
</script>
<?php
	echo_contents();
