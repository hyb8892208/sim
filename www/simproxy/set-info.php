...
function save_to_gw_info(){
	if(!file_exists('/opt/simbank/www/myimages/gw_info.conf')){
		exec('touch /opt/simbank/www/myimages/gw_info.conf');
	}
	
	$aql = new aql();
	$aql->set('basedir', '/opt/simbank/www/myimages/');
	...
	$img_dir = '/opt/simbank/www/myimages/';
	if(!is_dir($img_dir)){
		mkdir($img_dir);
	}
	...
	if($_FILES['header_image']['tmp_name'] != ''){
		if(isset($general_section['general']['header_image'])){
			$aql->assign_editkey('general','header_image','/myimages/header_image.png');
		}else{
			$aql->assign_append('general','header_image','/myimages/header_image.png');
		}
	}
	
	if($_FILES['footer_image']['tmp_name'] != ''){
		if(isset($general_section['general']['footer_image'])){
			$aql->assign_editkey('general','footer_image','/myimages/footer_image.png');
		}else{
			$aql->assign_append('general','footer_image','/myimages/footer_image.png');
		}
	}
...
function download_sample_images(){
	...
	$download_file = "/opt/simbank/www/images/Sample_Images.tar.gz";
	$pack_cmd = "tar -C /opt/simbank/www/images -cvf $download_file $header_path $footer_path";
...
if(!is_dir('/mnt/data/info/images')){
	mkdir('/mnt/data/info/images');
}
...
if (!$setok) {
	mkdir('/opt/simbank/www/myimages/');
}
			...
			<tr>
				<th>
					<div class="helptooltips">
						<?php echo language('Switch@setinfo','Switch');?>:
						<span class="showhelp">
							<?php echo language('Switch help@setinfo','If this switch is enabled, the following parameter information will be used.');?>
						</span>
					</div>
				</th>
				<td>
