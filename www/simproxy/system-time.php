...
function click(){
	...
	$.ajax({
		type:'GET',
		url: "/simproxy/ajax_server_simbank.php?random="+Math.random()+"&action=sync_time_from_ntp",
		...
		success: function(data) {
			...
			window.setInterval('window.location.href="/simproxy/system-time.php";',1000);
...
function check()
{
	...
	if(ntp_server1 != '') {
		if(!check_domain(ntp_server1)) {
			var rstr = "<?php echo language('js check domain','Invalid domain or IP address.');?>";
...
function save_data()
{
	if( isset($_POST['ntp_server1']) &&
		...
		$file_path="/config/etc/time.conf";
		...
		$cmd = "/bin/ln -sf /usr/share/zoneinfo/$zonefile /config/etc/localtime";
		system($cmd);
...
if(isset($res['general']['auto_sync'])) {
	$val = trim($res['general']['auto_sync']);
			...
			<td>
				<select id="system_timezone" style="width: 253px;" name="system_timezone" onchange="modechange(this)">
					...
					<optgroup label="<?php echo language('Asia');?> (UTC+9)">
						...
						<option  value="Asia/Pyongyang@KST-8:30"><?php echo language('Pyongyang');?></option>
					...
					<optgroup label="<?php echo language('Unknown');?>">
						<option value="-@UTC+0"><?php echo language('User defined @or out of date');?></option>
					</optgroup>
				</select>							
			</td>
		</tr>
...
?>
		<tr>
			<th>
				<div class="helptooltips">
					<?php echo language('Auto-Sync from NTP');?>:
					<span class="showhelp">
					<?php
						$help = 'Whether enable automatically synchronize from NTP server or not. On(enabled),OFF(disabled).';
						echo language('Auto-Sync from NTP help', $help);
					?>
					</span>
				</div>
			</th>
	...
	<input type="submit" value="<?php echo language('Save Data');?>" onclick="document.getElementById('send').value='Save Data';return check();" />
	<input type="button" value="<?php echo language('Sync from NTP');?>" onclick="sync_time_from_ntp();"/>
	<input type="button" value="<?php echo language('Sync from Client');?>" onclick="sync_time_from_client();"/>
...
function sync_time_from_client(){
	...
	$.ajax({
		type:"POST",
		url:"/simproxy/ajax_server_simbank.php?action=sync_time_from_client",
