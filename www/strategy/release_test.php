...
function simbank_release_handler()						//定期检测线路是否需要释放
{
	...
	if ($group_name == "") {
		...
		$condition = "WHERE ob_sb_seri IN (SELECT ob_sb_seri FROM tb_simbank_info WHERE ob_grp_name='$group_name') AND  n_sb_link_online=0 AND n_sb_link_stat=1 AND n_sb_link_call_time>" . $release_time;
	}
	$fields = "*";
	...
	while($simbank_data_release = mysqli_fetch_array($data,MYSQLI_ASSOC)) { 
		...
		echo "====================Sending Release msg[".$simbank_data_release['ob_gw_seri']."-".$simbank_data_release['ob_gw_link_bank_nbr']."-".$simbank_data_release['ob_gw_link_slot_nbr']."--".$simbank_data_release['ob_sb_seri']."-".$simbank_data_release['ob_sb_link_bank_nbr']."-".$simbank_data_release['ob_sb_link_sim_nbr']."]\n";
