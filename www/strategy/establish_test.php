...
$message = array(
	"new_version"=> 0001,
	"msgtype" => 0005,
	"msglen" => 76+2+$simbank_link_data['n_sb_link_vgsm_len'],
	"result" => 0,
	"reserve" => 0
);

$pkt = pack("nnnnna10nnnVnnH64a10nnVnnH".$simbank_link_data['n_sb_link_vgsm_len']*2, $message['new_version'], $message['msgtype'], $message['msglen'],$message['result'],$message['reserve'],$simbank_link_data['ob_sb_seri'],$simbank_link_data['ob_sb_link_usb_nbr'],$simbank_link_data['ob_sb_link_bank_nbr'],$simbank_link_data['ob_sb_link_sim_nbr'],$simbank_link_data['n_sb_link_ip'],$simbank_link_data['n_sb_link_port'],$simbank_link_data['n_sb_link_atr_len'],$simbank_link_data['b_sb_link_atr'],$gateway_link_data['ob_gw_seri'],$gateway_link_data['ob_gw_link_bank_nbr'],$gateway_link_data['ob_gw_link_slot_nbr'],$gateway_link_data['n_gw_link_ip'],$gateway_link_data['n_gw_link_port'],$simbank_link_data['n_sb_link_vgsm_len'],$simbank_link_data['b_sb_link_vgsm']);
...
{	
	...
	{
		/* 
		array("//第一步 查找配对过的网关线路同位置的同组的网关线路",
			  "//第二步 查找配对过的网关线路同位置的其他组的网关线路",
			  "//第三步 查找配对过的网关线路不同位置不同组的网关线路");
		*/
		$condition = array("WHERE ob_gw_link_slot_nbr=0 and ob_gw_seri IN (SELECT A.ob_gw_seri FROM tb_gateway_info AS A,(SELECT ob_grp_name,ob_loc_id FROM tb_gateway_info WHERE ob_gw_seri='$ob_gw_seri')B WHERE A.ob_grp_name = B.ob_grp_name AND A.ob_loc_id = B.ob_loc_id) AND n_gw_link_stat=0 AND ob_gw_link_slot_nbr!='$ob_gw_link_slot' LIMIT 1",
	...
	else				  //simbank线路未被配对过,处于初始状态
	{
		...
		*/	
		$condition = array("WHERE ob_gw_link_slot_nbr=0 and ob_gw_seri IN (SELECT ob_gw_seri FROM tb_gateway_info WHERE ob_grp_name = (SELECT ob_grp_name FROM tb_simbank_info WHERE ob_sb_seri='$ob_sb_seri')) AND n_gw_link_stat = 0 LIMIT 1",
