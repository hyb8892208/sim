...
/*---------------------------------------------------------------------------------
   ...
   实 例：无
-----------------------------------------------------------------------------------*/
public function __construct($db='tmp_simserver'){
     $this->host   = '127.0.0.1';
     $this->user   = 'root';
	 
	 global $cloudSimProxy;
...
private function connect(){
   ...
   else $this->conn=mysqli_connect($this->host,$this->user,$this->pwd); // 临时链接
   if ($this->show_error){
    if(!$this->conn) $this->show_error("无法连接服务器!");
    if(!$this->select_db($this->db)) $this->show_error("无法连接数据库：",$this->db);
   }
   mysqli_query($this->conn,"SET NAMES $this->char");
   mysqli_query($this->conn,"SET CHARACTER_SET_CLIENT='$this->char'"); 
   mysqli_query($this->conn,"SET CHARACTER_SET_RESULTS='$this->char'");
}

// 数据库选择
public function select_db($db){return mysqli_select_db( $this->conn,$db);}
...
public function Add($Table,$Fileds,$Value){
	...
	//echo "INSERT INTO $Table ($Fileds) VALUES ($Value)"."\n\r\n";
...
public function Set($Table,$Content,$Condition=""){
	//echo "UPDATE $Table SET $Content $Condition"."\n\r\n";
